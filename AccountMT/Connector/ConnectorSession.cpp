...
ConnectorSession::ConnectorSession(boost::asio::ip::tcp::socket socket) : _socket(std::move(socket))
{
	
}

void ConnectorSession::do_read()
{
...
void ConnectorSession::do_write(char *write_buffer, std::size_t length)
{
	auto self(shared_from_this());
...
void ConnectorSession::start()
{
	this->do_read();
}

void ConnectorSession::handle_packets()
{
